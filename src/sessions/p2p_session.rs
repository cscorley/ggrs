...
impl<T: Config> P2PSession<T> {
    ...
    /// This data is also sent uncompressed.  Additionally, spectators can only
    /// receive data from the host (e.g., the host does not forward messages
    /// received from other players).  Users requiring guaranteed delivery
    /// should consider implementing their own protocol on top of this that fits
    /// their needs (such as ACK their own messages), or using a separate
    /// socket.
